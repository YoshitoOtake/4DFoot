function [xmin, ...      % minimum search point of last iteration
      ...
      diagD_final, ... % axis ratio in the final generation
      median_m_best ... % (median of objection function values in the final generation) - (best function value)
	 ] = cmaes( ...
...
% %               end
% %               isInclude(k) = feval(varargin{2}.cmaes_inclusionTest, arxvalid(:,k), varargin{:});
% %               tries = tries + 1;
% %             end
% % %             fprintf('k = %d, tries = %d\n', k, tries);
%           end
%       end
      % --- end ---- added by YO (@2012/10/10)
...
if flgdisplay
  ...
  diagD_final = diagD;
  median_m_best = median(fitness.raw)-fitness.hist(1);
...
%
  if ~isempty(lbounds)
    if length(lbounds) == 1
      ...
      arbounds = lbounds*ones(1,size(x,2)); %repmat(lbounds, 1, size(x,2));
      idx = x < arbounds;
      x = max(arbounds, x);
    end
  else
  ...
  if ~isempty(ubounds)
    if length(ubounds) == 1
      ...
      arbounds = ubounds*ones(1,size(x,2)); %repmat(ubounds, 1, size(x,2));
      idx2 = x > arbounds;
      x = min(arbounds, x);
    end
  else
